...
unittest {
    // https://github.com/rdebath/Brainfuck/blob/master/testing/Golden.b
    auto goldenRatio = " ++++++[>++++++++<-]>+.---.>>>+>>+>++>+>+>+>>+>+++>+>>+>
+>+>>+>>>>+>>>>+>++>>>>+>+<>-----[+++++<[>>>]<<<[>>>>>>>>>+<<<<<<<<[>>>>>>>>>+<<
<<<<<<<-]<-<<<]>>>>>>+>>>+[<<<]<<<<<[>>>+<<[>>>+<<<-]<-<<<]>>>+<<<<<<<<[<<]>>>>[
->[<<<+>>>>[>>]>+<<<[<<]>-]<<<-[>>>+<<<-]+>>>>[>>]+>[-[-[-<->>>>>>[[>>>]>>]<<<<<
...
+>>]>+>[>+++++[>++>++<<-]<-]>>>>>[<<<+>+>+>-]<-[>+<-]+<<[<[<+>-]<<[>>+<<-]>[<+>-
    ...
    auto result = brainfuck(goldenRatio.to!(char[]), "".to!(char[]));
    assert(result.array == "1.618033988749894848204586834365638117");
}
